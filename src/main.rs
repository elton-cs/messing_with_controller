...
async fn main() {
    ...
    let guess: ByteArray = "STARK".into();
    let mut guess_felts = vec![];
    guess.encode(&mut guess_felts).unwrap();
    let make_guess = account.execute_v1(vec![Call {
        to: Felt::from_hex_unchecked(
            "0x2ccc78c905d9fa42a542b8de8c438559ebac7eaecedf5cd932ffbd46fd98729",
        ),
        selector: starknet::core::utils::get_selector_from_name("submit_guess").unwrap(),
        calldata: guess_felts,
    }]);
    let transaction2 = make_guess.send().await.unwrap();

    // let crosswordle = CrosswordleCrosswordle::new(
    //     Felt::from_hex_unchecked(
    //         "0x2ccc78c905d9fa42a542b8de8c438559ebac7eaecedf5cd932ffbd46fd98729",
    //     ),
    //     account,
    // );
    // let transaction1 = crosswordle.start_game().call().await.unwrap();
}
