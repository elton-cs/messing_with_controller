...
#[tokio::main]
async fn main() {
    // let owner = Signer::Starknet(SigningKey::from_secret_scalar(Felt::from_hex_unchecked(
    //     PRIVATE_KEY,
    // )));
    ...
    let start_game = account.execute_v1(vec![Call {
    ...
    let guess_str: ByteArray = "SWORD".into();
    let mut encoded = vec![];
    guess_str.encode(&mut encoded).unwrap();
    let make_guess = account.execute_v1(vec![Call {
        to: Felt::from_hex_unchecked(
            "0x2ccc78c905d9fa42a542b8de8c438559ebac7eaecedf5cd932ffbd46fd98729",
        ),
        selector: starknet::core::utils::get_selector_from_name("submit_guess").unwrap(),
        calldata: encoded,
    }]);

    // let result = start_game.send().await.unwrap();
    let result = make_guess.send().await.unwrap();
}
